    ...
    const rows = await prisma.$queryRaw<ItemRow[]>`
      ...
      lines AS (
        ...
        WHERE ${saleBasis("sd")}
          ...
          -- One row per SET. The outdoor half of a split air conditioner scores
          -- on its indoor partner, so listing it here showed the seller every
          -- machine twice — once with its points and once with a bare zero.
          -- A half with no partner on the bill scores itself and stays.
          AND NOT (
            COALESCE(cat.pointmap_category, '') = 'Air'
            AND sd.item_name ~ '\\[H\\]\\s*$'
            AND ${incentiveMatePrice("sd")} IS NOT NULL
          )
      ),
      scored AS (
