...
// price). No joins to brand/category/group lookup tables, no set-detail
// EXISTS subqueries, no stock-balance function calls. Per-warehouse stock
// is fetched by a separate `/api/inventory/stock-balance` endpoint when
// the user actually picks a product — keeps this list query fast even
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json(
    rows.map((r) => ({
      ...
      unit: r.unit_standard_name?.trim() || null,
      nameLo: r.name_1?.trim() || r.code,
      nameEng: r.name_eng_1,
      unitName: r.unit_standard_name?.trim() || null,
      brand: r.item_brand,
      brandName: null,
