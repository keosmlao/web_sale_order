...
export default function MyBonusCard() {
  ...
  // ມື້ນີ້ (today only) / ສະສົມ (whole month) tabs — today first, each list
  // fetched lazily the first time its tab is shown.
  const [itemsTab, setItemsTab] = useState<"month" | "today">("today");
  ...
  // ---- shared blocks (rendered in both the mobile combined card and the
  // desktop split cards) ----

  // HEADLINE: the total to receive, styled like a banknote — ornate frame,
  // denomination in the corners, guilloche rings, serial no.
  const banknote = (
    <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-emerald-800 via-emerald-900 to-teal-950 p-1.5 shadow-md">
      ...
      <div className="relative rounded-lg border border-dashed border-emerald-200/40 px-3 pb-3 pt-2.5">
        ...
        <span className="absolute left-2 top-1.5 font-mono text-[11px] font-black text-emerald-200/70">{fmt.format(row.totalPay)}</span>
        <span className="absolute right-2 top-1.5 font-mono text-[11px] font-black text-emerald-200/70">{fmt.format(row.totalPay)}</span>
        <span className="absolute bottom-1.5 left-2 font-mono text-[11px] font-black text-emerald-200/70">{fmt.format(row.totalPay)}</span>
        <span className="absolute bottom-1.5 right-2 font-mono text-[11px] font-black text-emerald-200/70">{fmt.format(row.totalPay)}</span>
        <div className="text-center text-white">
          <div className="text-[9px] font-bold tracking-[0.35em] text-emerald-300/90">
            ✦ ODIEN GROUP ✦
          </div>
          <div className="mt-0.5 text-xs font-black uppercase tracking-widest text-emerald-200">
            ລວມທີ່ຕ້ອງຮັບ
          </div>
          <div className="mt-0.5 flex items-baseline justify-center gap-1.5">
            ...
            <span className="text-sm font-black text-emerald-200/90">{currencyLabel}</span>
          </div>
          <div className="mt-1 text-xs font-bold text-emerald-100/80">
            {isRoleComm
              ? `ຄ່າຄອມຕາມຜົນງານທີມ`
              : `ໂບນັດ ${fmt.format(bonusTotal)} + ຄ່າຄອມ ${fmt.format(row.commission)}`}
          </div>
          <div className="mt-1 font-mono text-[9px] font-bold tracking-[0.25em] text-emerald-300/60">
  ...
  // Points strip — today's points FIRST, then the month's running total.
  const pointsRow = (
    <div className="mt-3 grid grid-cols-2 divide-x divide-slate-200 overflow-hidden rounded-xl border border-slate-200 bg-slate-50/70">
      ...
      <div className="flex items-center justify-center gap-2 px-2 py-2">
        ...
        <span className="text-[11px] font-black text-slate-500">ສະສົມ</span>
        <span className="font-mono text-lg font-black text-emerald-700">{pointFmt.format(row.bonusPoints)}</span>
      </div>
    </div>
  );

  // How the commission is worked out — the caller's OWN numbers first
  // (base × rate = amount), then the 3 tier rules with concrete examples so
      ...
      {row.commissionLines && row.commissionLines.length > 0 ? (
        <div className="mt-2 space-y-1">
          {row.commissionLines.map((l) => (
            <div key={l.groupCode} className="flex items-center justify-between rounded-lg bg-white px-2.5 py-1.5 font-mono text-[13px] font-bold ring-1 ring-inset ring-indigo-100">
              <span className="text-slate-500">
                {GROUP_LABEL[l.groupCode] ?? l.groupCode}
                <span className="ml-1 text-[11px] text-slate-400">ທີມບັນລຸ {pctFmt.format(l.achievementPct * 100)}%</span>
              </span>
              <span>
          ...
          <div className="flex items-center justify-between rounded-lg bg-slate-900 px-2.5 py-1.5 font-mono text-[13px] font-black text-white">
            <span>ລວມຄ່າຄອມ</span>
        ...
        <>
          <div className="mt-2 flex flex-wrap items-center justify-center gap-x-1.5 gap-y-1 rounded-lg bg-white px-2 py-2.5 text-center font-mono text-sm font-black ring-1 ring-inset ring-indigo-100">
            ...
            <span className="text-[11px] font-bold text-slate-400">{currencyLabel}</span>
          </div>
  ...
  const tierSection = row.targetPerPerson > 0 && tiers ? (() => {
    ...
    return (
      <div className="mt-3 rounded-xl bg-slate-50 p-3 ring-1 ring-inset ring-slate-100">
        ...
        <div className="relative mt-1 h-3 text-[10px] font-bold text-slate-400">
        ...
        <div className="mt-1 flex justify-between text-xs font-black">
        ...
        {(() => {
          ...
          return (
            <div className="mt-2 space-y-1">
              {gaps.map((g) => (
                <div key={g.label} className="flex items-center justify-between rounded-lg bg-white px-2.5 py-1.5 text-xs font-bold ring-1 ring-inset ring-slate-100">
                  <span className="text-slate-500">
  ...
  const dailyChart = daily.length > 0 && !isRoleComm ? (() => {
    const now = new Date();
  ...
  const itemsButton = (
    <button type="button" onClick={() => void toggleItems()} className="mt-4 flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-sm font-black text-emerald-700 transition active:scale-[.98] hover:bg-emerald-100">
    ...
    <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="grid grid-cols-2 gap-1 border-b border-slate-100 bg-slate-50 p-1">
        <button
          ...
          className={`rounded-lg px-3 py-1.5 text-[13px] font-black transition ${itemsTab === "today" ? "bg-white text-indigo-700 shadow-sm ring-1 ring-slate-200" : "text-slate-400"}`}
        >
          ມື້ນີ້
        </button>
        <button
          ...
          className={`rounded-lg px-3 py-1.5 text-[13px] font-black transition ${itemsTab === "month" ? "bg-white text-emerald-700 shadow-sm ring-1 ring-slate-200" : "text-slate-400"}`}
        >
          ສະສົມ (ເດືອນນີ້)
        </button>
      </div>
      {items === null ? (
        <div className="py-6 text-center text-sm text-slate-400">ກຳລັງໂຫລດ…</div>
      ) : items.length === 0 ? (
  ...
  const header = (title: string, subtitle: string, tone: "emerald" | "indigo" = "emerald") => (
    <div className={`flex items-center justify-between border-b px-4 py-3.5 ${tone === "indigo" ? "border-indigo-100/70 bg-gradient-to-r from-indigo-50 to-violet-50/60" : "border-emerald-100/70 bg-gradient-to-r from-emerald-50 to-teal-50/60"}`}>
      <div className="flex items-center gap-2.5">
        <span className={`flex h-9 w-9 items-center justify-center rounded-xl text-white shadow-sm ${tone === "indigo" ? "bg-indigo-600 shadow-indigo-300" : "bg-emerald-600 shadow-emerald-300"}`}>
        ...
        <div><div className="text-base font-black text-slate-900">{title}</div><div className="text-xs font-semibold text-slate-500">{subtitle}</div></div>
      </div>
    </div>
  );

  // Bonus and commission live in TWO separate cards under a shared
  // full-width "total to receive" banknote strip — the SAME layout for every
  // role and every screen size. Phones stack the cards; on lg+ the wrapper
  // spans 2 of the home grid's 3 columns and puts them side by side.
  // Managers / unit heads simply have no bonus card, so their commission
  // card takes the full row.
  return (
    <div className="grid grid-cols-1 items-start gap-4 sm:mt-3 lg:col-span-2 lg:grid-cols-2">
      <div className="lg:col-span-2">{banknote}</div>

      {/* ໂບນັດ card — points earned this month */}
      {isRoleComm ? null : (
        <section className="overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-[0_10px_35px_-18px_rgba(5,150,105,0.4)]">
          {header("ໂບນັດ", "ຄະແນນສະສົມ × ຕົວຄູນ")}
          <div className="p-4">
            <div className="rounded-xl bg-gradient-to-br from-emerald-600 to-emerald-700 p-3 text-white shadow-sm">
              <div className="flex items-center justify-between gap-1">
                <span className="text-xs font-black text-emerald-100">ໂບນັດເດືອນນີ້</span>
            ...
            {dailyChart}
            {itemsButton}
            {open ? itemsPanel : null}
          </div>
        </section>
      )}

      {/* ຄ່າຄອມ card — target-achievement (or team-performance) commission */}
      <section className={`overflow-hidden rounded-2xl border border-indigo-100 bg-white shadow-[0_10px_35px_-18px_rgba(79,70,229,0.45)] ${isRoleComm ? "lg:col-span-2" : ""}`}>
        {header("ຄ່າຄອມ", isRoleComm ? "ຕາມຜົນງານທີມ" : "ຕາມຍອດບັນລຸເປົ້າ", "indigo")}
        <div className="p-4">
          <div className="rounded-xl bg-gradient-to-br from-indigo-600 to-indigo-700 p-3 text-white shadow-sm">
            <div className="flex items-center justify-between gap-1">
              <span className="text-xs font-black text-indigo-100">ຄ່າຄອມເດືອນນີ້</span>
            ...
            <div className="mt-1.5 flex items-baseline gap-1.5">
              <span className="font-mono text-3xl font-black leading-none">{fmt.format(row.commission)}</span>
              <span className="text-xs font-black text-indigo-100">{currencyLabel}</span>
            </div>
          </div>
          {commissionCalc}
          {isRoleComm ? tierSection : null}
        </div>
      </section>
    </div>
  );
